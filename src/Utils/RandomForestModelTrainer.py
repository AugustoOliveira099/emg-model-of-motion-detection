...
class RandomForestModelTrainer:
    ...
    def train(self, X_train, y_train):
        ...
        # }
        self.model = RandomForestClassifier(max_depth= 10, max_features= "sqrt", min_samples_leaf= 5, min_samples_split= 2, n_estimators= 100, random_state=3)
    ...
    def load_model(self, path='data/rf_model.pkl'):
                ...
                for stage in os.listdir(patient_path):
                    ...
                    if os.path.isdir(stage_path):
                        for movement_file in os.listdir(stage_path):
                            movement_path = os.path.join(stage_path, movement_file)
                            try:
                                ...
                                print(f"Error decoding JSON in file {movement_path}: {e}")
        return pd.DataFrame(all_features)
    
    def predict_and_plot(self, signal, signal_features):
        predictions = {}
        times = {}
        min = 0
        max = 0
        for muscle in signal_features:
            ...
            if newMin < min:
                min = newMin
            if newMax > max:
                max = newMax + newMax * 20/100 # Salva com uma margem de 20% para melhor visualização
        ...
        # for muscle, preds in predictions.items():
            ...
            for i in range(1, len(preds)):
                if preds[i] != preds[i - 1]:
                    ...
                    with open(f'assets/{muscle}_movement_times.txt', 'a') as f:
            ...
            plt.savefig(f'assets/{muscle}_movement_predictions.png')
            plt.close()
